...
app.post("/networth", async (req, res) => {
    try {
        const results = await db.query ("INSERT INTO networth (name, value, base_value, investment) values ($1,$2,$3,$4) RETURNING *",
...
app.delete("/networth/:id", async (req, res) => {
    try {
        const results = await db.query("DELETE FROM networth where id =$1", [req.params.id])
        res.status(204).json({
            status: "success",
        })
...
app.put("/networth", async (req, res) => {
    try {
        const results = await db.query("UPDATE networth SET name=$1, value=$2, base_value=$3, investment=$4 WHERE id=$5 RETURNING *", 
        [req.body.name, req.body.value, req.body.base_value, req.body.investment, req.body.id])
        res.status(200).json({
            status: "success",
            data: {
                networth: results.rows[0]
...
app.post("/networth/time", async (req, res) => {
    try {
        const results = await db.query ("INSERT INTO networth_time (total_networth, monthly_income, investment_profit, monthly_profit) values ($1,$2,$3,$4) RETURNING *",
        [req.body.total_networth, req.body.monthly_income, req.body.investment_profit, req.body.monthly_profit])
        res.status(201).json({
            status: "success",
            data: {
